...
FTM_Type * dimFtms[12] = { BOARD_INITPINS_CH1_DIM_PERIPHERAL,
		BOARD_INITPINS_CH2_DIM_PERIPHERAL, BOARD_INITPINS_CH3_DIM_PERIPHERAL,
		BOARD_INITPINS_CH4_DIM_PERIPHERAL, BOARD_INITPINS_CH5_DIM_PERIPHERAL,
		BOARD_INITPINS_CH6_DIM_PERIPHERAL, BOARD_INITPINS_CH7_DIM_PERIPHERAL,
		BOARD_INITPINS_CH8_DIM_PERIPHERAL, BOARD_INITPINS_CH9_DIM_PERIPHERAL,
		BOARD_INITPINS_CH10_DIM_PERIPHERAL, BOARD_INITPINS_CH11_DIM_PERIPHERAL,
		BOARD_INITPINS_CH12_DIM_PERIPHERAL };

uint8_t dimChs[12] = { BOARD_INITPINS_CH1_DIM_CHANNEL,
		BOARD_INITPINS_CH2_DIM_CHANNEL, BOARD_INITPINS_CH3_DIM_CHANNEL,
		BOARD_INITPINS_CH4_DIM_CHANNEL, BOARD_INITPINS_CH5_DIM_CHANNEL,
		BOARD_INITPINS_CH6_DIM_CHANNEL, BOARD_INITPINS_CH7_DIM_CHANNEL,
		BOARD_INITPINS_CH8_DIM_CHANNEL, BOARD_INITPINS_CH9_DIM_CHANNEL,
		BOARD_INITPINS_CH10_DIM_CHANNEL, BOARD_INITPINS_CH11_DIM_CHANNEL,
		BOARD_INITPINS_CH12_DIM_CHANNEL };
...
int main(void) {
	...
	ftm_chnl_pwm_signal_param_t ftmParams[6] = { ftmParam, ftmParam, ftmParam,
			ftmParam, ftmParam, ftmParam };

	for (i = 0; i < 6; i++) {
		ftmParams[i].chnlNumber = i;
	}

	FTM_SetupPwm(FTM0, &ftmParams, 6U, kFTM_EdgeAlignedPwm, 2400U,
			FTM_SOURCE_CLOCK);
	FTM_SetupPwm(FTM1, &ftmParams, 2U, kFTM_EdgeAlignedPwm, 2400U,
			FTM_SOURCE_CLOCK);
	FTM_SetupPwm(FTM2, &ftmParams, 2U, kFTM_EdgeAlignedPwm, 2400U,
			FTM_SOURCE_CLOCK);
	FTM_SetupPwm(FTM4, &ftmParams, 2U, kFTM_EdgeAlignedPwm, 2400U,
			FTM_SOURCE_CLOCK);
